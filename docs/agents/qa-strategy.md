...
4. When an existing test assembly is available, isolate discovery and testhost plumbing with `dotnet test Werewolves.Core/Werewolves.Core.Tests/Werewolves.Core.Tests.csproj --no-build --no-restore --list-tests`. A successful no-build discovery or test run is not current-source evidence unless a successful build has first established that the assembly is fresh; record the discovered test inventory or count so stale assemblies are visible.
5. With both variables unset so MSBuild uses its normal in-process node, run an explicit `dotnet build <project> --no-restore`, then run `dotnet test <project> --no-build --no-restore`. Only interpret the no-build result as current-source evidence after that freshness proof, and record the executed test count. Give each command a caller-enforced time limit and run only one build or test probe at a time.
6. If an original `dotnet test` command with an implicit build hangs while both out-of-process-node variables are absent, run one isolated explicit build for that project, then retry the exact original command unchanged once. The comparison can distinguish a transient implicit-build orchestration or cold-output race from a deterministic Build-target or testhost failure, but it does not by itself prove a root cause. A caller timeout or wrapper exit does not prove that its descendants exited: inspect the full exact-owned process tree or process group, including `dotnet`, MSBuild workers, testhost, and `VBCSCompiler`, and terminate only proven-owned descendants or the exact owned process group before retrying; `pgrep -x dotnet` alone is insufficient.
7. If the normal full-solution build reaches MAUI/iOS task-host work and then stalls or fails inside the sandbox, retry the exact unchanged `dotnet build Werewolves.sln --no-restore` command with the required sandbox escalation. Do not add MSBuild diagnostic overrides to compensate for a sandbox IPC boundary.
8. On this macOS runner, a successful escalated focused build or test can exit cleanly while its `/nodeReuse:true` MSBuild worker is reparented to PID 1 and remains alive. Repeating one build per TDD tracer can therefore accumulate idle workers even though every root `dotnet build` or `dotnet test` has finished. For focused red-green probes, use the supported `dotnet test <project> ... --disable-build-servers` option and verify after its first use that the exact process tree contains no surviving `MSBuild.dll` or `VBCSCompiler` worker. This focused-probe mitigation does not change the normal final build/test commands in step 5. If workers from earlier probes already exist, first prove that no build or test is active anywhere, then run one `dotnet build-server shutdown`; terminate only individually proven-owned survivors and recheck the full tree.

> **Client test host note:** Browser QA host tests can hang in `WebApplication.CreateBuilder` on this host unless configuration reload is disabled.
> After a fresh build, run `DOTNET_HOSTBUILDER__RELOADCONFIGONCHANGE=false dotnet test Werewolves.Client.Tests/Werewolves.Client.Tests.csproj --no-build --no-restore`.
...
Canonical identity tests should prefer structural assertions and round-trip or parse checks over broad exact string assertions. Assert role counts by enum value, sort order by enum identifiers, zero-count omission, Actor Setup Card exclusion, `players=N`, and non-default assumptions as structured fields. For a Thief-enabled `P + 2` Role Composition, also assert a `P`-card Deal Pool with exactly one Thief, two distinct configured offer slots backed by different inventory identities with no offered Thief, and partition-sensitive Canonical Simulation Scenario identity. Distinct offer slots must remain distinct even when their behaviorally identical screening branches deduplicate. Use shared constants or value objects for separators, labels, profile ids, and version ids where they exist. Exact literal string assertions are allowed only at the narrow serializer/cache-key boundary when the literal format itself is intentionally the public or cache contract.
...
Thief setup and runtime evidence must stay layered. Use value tests for the `P + 2`/Deal Pool/offer partition, inventory conservation, and distinct configured offer slots. Simulation Start State derivation tests must prove that only Deal Pool cards receive seeded assignments while ordered `Offer1` and `Offer2` remain in the derived state and Canonical Simulation Scenario identity; they must never construct a different executable Game Session by dropping the offers. Genuine offer-bearing public-Core or headless Game Session execution evidence begins only once Thief is admitted to the supported Role catalog. At that runtime layer, public-API Core integration tests prove exactly one Thief holder exists, conditional setup is required for Roles found only in offers, and committed choice/recovery cannot repeat an exchange or reopen `Decline`; rendered client tests prove the private surface shows only Core-provided `Offer1`, `Offer2`, and legal `Decline` options without asking the Moderator to re-enter the locked pair. When same-printed cards are bound to Players, prove unique matching accounting identities and one-owner/one-zone conservation without asserting that the Moderator observed one particular copy or relying on incidental allocation order. Prove that an exchange makes the selected offer Player-owned and moves the original Thief card plus the unchosen offer to Set-Aside, while `Decline` retains Thief and moves both offers to Set-Aside. Recovery evidence must kill and rehydrate immediately after successful response processing and prove ADR-0017's atomic stable checkpoint restores the resulting zones, Role/fresh state, and pending sleep instruction without replaying the response.
...
| `Werewolves.Client.Tests.Documentation.WerewolfAgentGroupObservationContractTests.WerewolfAgentGroupObservationDocs_DefineExactInitialAllAlivePrivateFailClosedContract` | Permanent policy | CONTEXT, ADR-0019, and the Moderator interaction contract keep exact initial all-alive Faction Agent group observation, fail-closed unsupported-state behavior, Role non-identification, and audience boundaries aligned. | Markdown is the policy surface; keep one narrow multi-document contract test. |
...
| `Werewolves.Client.Tests.Documentation.CompositionRootOwnershipTests.CompositionRoots_DelegateSessionAndLobbyGraphToSharedModule` | Permanent policy | MAUI, Browser QA, and bUnit composition roots delegate the shared session-and-Lobby graph with their host-specific lifetime and Lobby factory choices. | The three composition-root source files are the architecture ownership contract; keep this narrow structural check paired with runtime provider tests. |
