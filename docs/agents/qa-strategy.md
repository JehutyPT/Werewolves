...
- Use `/?qa=lobby`, `/?qa=probability`, `/?qa=degenerate`, `/?qa=dashboard`, and `/?qa=victory` for deterministic access to ordinary role selection, a dormant-probability-record negative check, the retained degenerate warning, dashboard/action instruction, and victory states.
