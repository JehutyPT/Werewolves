...
5. With both variables unset so MSBuild uses its normal in-process node, run the normal build and then `dotnet test --no-build`. Give each command a caller-enforced time limit and run only one build or test probe at a time. Before retrying, confirm that the caller's prior owned process group has exited; if it has not, stop that exact group first.
