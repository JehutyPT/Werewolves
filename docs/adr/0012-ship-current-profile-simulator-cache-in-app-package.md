...
ADR-0016 narrows the preceding compatibility statement. A legacy `core-simulator@1` probability record may satisfy the current `safety-screening@<version>` consumer only through an explicit bridge for a scenario in the intersection of that legacy producer/profile and the current Safety-Screening capability, with exact Role, setup, outcome, and headless-response semantics declared compatible. Newly admitted safety-only Roles and changed semantics cannot use the bridge, and the existing artifact is not re-keyed or regenerated by PRD #93.
