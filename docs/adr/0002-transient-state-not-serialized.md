...
On Rehydration, the cascade stage derives its remaining waves from these semantic facts. It does not re-run a settled zero-elimination interception, re-admits recorded child eliminations without repeating completed reaction side effects, and does not reuse a completed Dawn or Vote scope. The live wave queue, reaction bindings, active stage cursor, and listener state remain transient and are never serialized.
