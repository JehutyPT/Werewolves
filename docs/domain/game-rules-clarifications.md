...
- A Role Composition from which a Role with a different initial Beneficiary is reachable is not App-Supported until that Role's establishment path is available. Angel is not such an exception: its holder uses ordinary Villager **Faction Beneficiary** mechanics throughout, while Angel's **Role-Card Victory Eligibility** is evaluated separately from Initial Beneficiary Closure.
...
- At a **Victory Check Window**, the Prejudiced Manipulator Faction predicate is true only if a living Prejudiced Manipulator **Faction Beneficiary** exists and either partition group contains no living Player. A living beneficiary necessarily occupies one group, so that group cannot be empty; the test is therefore equivalent to checking that beneficiary's **Opposing Public Group** is empty and does not require selecting a side from the holder's identity at that window. An Eliminated Manipulator cannot win posthumously, including when the Manipulator and the final opposing Player are Eliminated in the same cascade. Any other Faction predicate true in the same window still produces a **Shared Victory Outcome** under the general outcome rule.
- The simpler Victory Check does not remove the need to establish the exact holder early. The Night 1 call-order entry identifies an otherwise-unknown Deal Pool holder exactly once. A Thief choice or another committed transition that already establishes the current holder omits the slot, as does a branch that leaves the card Set-Aside.
- This identification closes the Player, Character Card, current Role, and default **Faction Beneficiary** facts before later Player-scoped Lovers classification, infection, Elimination, and **Permanent Role Swap** rules consume or replace them. It is an identification-only call, not a **Role Power**, and adds no gameplay choice; later holder changes are learned through their ordinary transition rather than another Prejudiced Manipulator identification slot.
...
- If nobody acts, a one-way Continue resumes Vote resolution against the unchanged target. Continue is not a Decline, identifies nobody, changes no Role or Character Card fact, and leaves the Devoted Servant power available for a later qualifying Vote.
- Use begins only when an eligible non-Lover Servant publicly reveals their own Character Card. Accepting that reveal spends the one-use power and durably commits the private swap-recording path; recovery cannot reopen Continue or repeat the reveal.
- On that path, the public Devoted Servant card moves to **Discarded** rather than face-down Set-Aside. The Servant takes the fixed Vote Target's still-unrevealed Character Card and reads its printed Role privately with the Moderator. The acquired current Role and card remain hidden from the rest of the table, while the former Devoted Servant reveal and discard remain public. The Permanent Role Swap starts fresh power state and removes the transferred Role and its role-bound behavior from the Vote Target.
- The transferred Role's target-side reveal, protection, pardon, immunity, and on-Elimination behavior do not run. In particular, an unused Village Idiot pardon transfers fresh to the Servant, while the original Vote Target is Eliminated by the same Vote without an Idiot reveal, pardon spend, or voting-power loss.
...
- The Servant inherits none of the Vote Target's relationships, Status Effects, listener state, or spent Role history. A Lover cannot use Devoted Servant, so no Lover relationship crosses a successful swap.
- The swap deliberately clears Charmed, Sheriff, and Town Crier as named parts of the Servant's old identity. Clearing Sheriff removes only its added **Durable Voting Power**; independent voting-right, voting-power, and restriction facts remain. This is specific to Devoted Servant, not a universal reset of every Status Effect.
...
- An unexpired Angel card transfers its remaining **Role-Card Victory Eligibility** with the physical card. Elimination of the new holder at any remaining qualifying boundary through Night 2—including during Night 2—qualifies Angel; eliminating the former Vote Target after the transfer does not. After the Dawn Night 2 expiry boundary, the printed Angel card still transfers but supplies Simple Villager current-Role mechanics.
- Once the private printed-Role record is accepted, recovery resumes resolution of the same Vote Target without repeating the public reveal, physical exchange, resource spend, Role or Faction transition, state clearing, or Elimination. A stale response changes nothing.
...
- Every actual Elimination of the physical Angel card's holder during Day 1 qualifies Angel, including Elimination by the first Vote, a **Consecutive Vote**, or any reaction or **Elimination Cascade**. Eligibility remains pending until all committed Votes and their complete cascades finish, then participates in the shared pre-Night **Victory Check Window**.
...
- During one **Victory Check Window**, all win-condition predicates are evaluated against the same resolved Game Session state. If multiple predicates are true, the Game Session ends with a **Shared Victory Outcome** rather than applying a priority order.
- Angel's **Role-Card Victory Eligibility** becomes true when the physical Angel card's holder is actually Eliminated during Night 1, any part of Day 1, or Night 2. Standard and **Consecutive Votes** and reaction or **Elimination Cascade** causes all qualify. The ordinary public reveal and ownership observation establish that the Eliminated Player held the card before the next shared **Victory Check Window** consumes the eligibility; Angel adds no identification flow, and its holder uses ordinary Villager **Faction Beneficiary** mechanics throughout. If Angel has not won by the Dawn **Victory Check Window** that resolves Night 2, the eligibility expires immediately after that window. The physical Angel card thereafter supplies Simple Villager current-Role mechanics without changing the card, its ownership, or its visibility; an unknown holder stays unknown, and a later ordinary reveal or ownership observation projects Simple Villager mechanics without retroactive identification.
- A **No-Winner Outcome** occurs only when no win condition is true in the **Victory Check Window** and every Player is Eliminated.
