...
This document records stable domain rules that implementation and tests can assert without carrying the rationale for why those rules were chosen. Use `CONTEXT.md` for vocabulary, `docs/game-rules-clarifications.md` for rule disambiguation, `docs/adr/` for architectural tradeoffs, and `docs/handoff.md` for the active simulator design ledger.
