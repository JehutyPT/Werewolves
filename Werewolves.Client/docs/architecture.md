...
*   **Complete Lobby Exit attempt:** `GameClientManager.AttemptLobbyExit` owns ordinary setup validation, transactional implicit Role Lock-In acceptance, required Actor Setup Cards then Public Group Partition progression, exact-current evaluation authorization, and durable start. It uses the composed Lobby and evaluator and returns host-agnostic semantic outcomes. Role Selection renders the outcome; Routes maps required configuration or successful start to its destination. Pending evaluation accelerates fallback and returns without starting; evaluation completion only updates presentation. Prepared Browser QA and test Game Sessions use explicitly separate fixture access.
...
*   **Recent setups:** `IRecentSetupStore` is a second, independent store containing only ordered Player names, normalized Role counts, and capture times. `Werewolves.Client.Shared` owns the stateless recent-setup collection policy; store adapters retain capture-time acquisition through `TimeProvider` and persistence I/O, including filesystem access. A successful Lobby Exit captures through this seam only after the blocking start/recovery/publication boundary has completed, and that final capture is best-effort. Loading a recent setup reuses the existing atomic Lobby decision/persistence/publication boundary with fresh Player identities and no accepted artifacts; deletion commits the recent-setups store before the Landing row is removed. The native file store uses its own current-schema payload and atomic file, never the Game Session recovery slot.
...
*   **Initial active save failure:** Implicit Role Lock-In acceptance and active recovery persistence are separate commitments. An active recovery-write failure discards the unexposed Core Game Session while retaining the latest successfully staged Lobby and replaceable Role Lock-In. Role Selection shows one resource-backed Portuguese inline alert beside ordinary Start. A subsequent attempt replaces the alert with its own outcome; Role Composition or accepted setup changes and departure clear it. Evaluation completion does not clear the alert or retry persistence. Earlier staging failure keeps its existing non-navigating presentation; recent-setup capture failure after successful publication remains nonblocking.
